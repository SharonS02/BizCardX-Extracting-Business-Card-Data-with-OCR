...
if selected=="Upload":
    ...
    if bz_cd != None:
        ...
        create=st.button(":red[Upload to Mysql]")
        if create:
            with st.spinner("Kindly wait for few mins"):
                insert='''insert into bixcard(name,
                                        designation,
                                        company_name,
                                        contact,
                                        alternative,
...
#:Update/Delete:        
        ...
        if selected1 =="UPDATE":
            ...
            if update:
                ...
                qr='''update bixcard set name = %s,designation = %s,company_name = %s,contact = %s,alternative = %s,
        ...
        if selected1 == "DELETE":
            list=cursor.execute("select name from bixcard")
            ...
            if select_name and delete:
                st.spinner("Loading")
                del_Qr= f"delete from bixcard where name='{select_name}'"
...
if selected=="Home":
    ...
    image=("C:\\Users\\manue\\Downloads\\Business-Cards-extraction-2.png")
